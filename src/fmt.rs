...
impl FromStr for Complex<f32> 
{
    type Err = ParseFloatError;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let float_str = r"^([+-]?(?:\d+|\d*\.\d+|\d+\.\d*)(?:[eE][+-]?\d{1,3})?)?";
        let float_imag_str = r"(?:([+-]?(?:\d+|\d*\.\d+|\d+\.\d*)(?:[eE][+-]?\d{1,3})?)[iI])?$";
        ...
        let x = textx.parse::<f32>();
        let y = texty.parse::<f32>();
        
        match (x, y) {
            (Ok(re), Ok(im)) => Ok(Self { re: re, im: im}),
            (Ok(re), Err(_)) => Ok(Self { re: re, im: 0.0_f32}),
            (Err(_), Ok(im)) => Ok(Self { re: 0.0_f32, im: im}),
            (Err(e), Err(E)) => Err(e),
        }
    }
}
